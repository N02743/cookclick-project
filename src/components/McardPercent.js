...
const MCard = ({
  ...
  let linkwarp = ""
  let classname
  if (parseFloat(Percent).toFixed(2) > 75) {
    classname = "percentorpersent percentgreen"
  } else if (parseFloat(Percent).toFixed(2) > 50) {
    classname = "percentorpersent percentdarkgreen"
  } else if (parseFloat(Percent).toFixed(2) > 25) {
    classname = "percentorpersent percentbrown"
  } else {
    classname = "percentorpersent percentred"
  }
    ...
    <div className="flex menu-card-can-delete">
      ...
      >
        <Modal.Body className="text-center" style={{ fontSize: "28px" }}>
          ยืนยันลบสูตรอาหาร <span>"{FoodName}"</span>
        </Modal.Body>
